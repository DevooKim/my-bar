...
@MainActor
enum Updater {
    private static let repo = "DevooKim/my-bar"
    private static let latestAPI = "https://api.github.com/repos/DevooKim/my-bar/releases/latest"
    ...
    private static func fetchLatest() async throws -> Release {
        ...
        request.setValue("MyBar-Updater", forHTTPHeaderField: "User-Agent")
    ...
    private static func downloadAndInstall(url: URL, version: SemanticVersion) async {
        do {
            ...
            let workDir = fm.temporaryDirectory.appendingPathComponent("MyBarUpdate-\(version.description)")
